...
const scrapeEvents = async () => {
    try {
        ...
        await page.waitForSelector('.discover-vertical-event-card', { timeout: 10000 });
        ...
        const events = await page.evaluate(() => {
            ...
            return eventCards.map(card => {
                ...
                // Get date
                const dateElement = card.querySelector('section.event-card-details p.Typography_body-md-bold__487rx');
                ...
                // Get location
                const locationElement = card.querySelector('p[class*="Typography_body-md"][class*="event-card__clamp-line--one"]');
                const location = locationElement?.innerText || 'Sydney, Australia';
                
                // Get price
                const priceElement = card.querySelector('.DiscoverVerticalEventCard-module__priceWrapper___usWo6 p');
